...
ShellExecTool::ShellExecTool(act::infrastructure::IProcess &proc,
...
QJsonObject ShellExecTool::schema() const
{
    ...
    props[QStringLiteral("command")] = [] {
        ...
        obj[QStringLiteral("description")] =
            QStringLiteral("The shell command to execute");
        return obj;
    }();
    ...
    props[QStringLiteral("args")] = [] {
        ...
        obj[QStringLiteral("description")] =
            QStringLiteral("Optional arguments for the command");
        return obj;
    }();
...
act::core::ToolResult ShellExecTool::execute(const QJsonObject &params)
{
    ...
    {
        return act::core::ToolResult::err(
            act::core::errors::COMMAND_BLOCKED,
    ...
    QEventLoop loop;
    m_proc.execute(command, args,
        [&output, &exitCode, &loop](int code, QString out) {
            exitCode = code;
            output = std::move(out);
            loop.quit();
        },
    ...
    if (exitCode != 0)
    {
        return act::core::ToolResult::err(
            ...
            QStringLiteral("Command exited with code %1: %2")
                .arg(exitCode)
                .arg(output.left(500)),
            metadata);
    }
