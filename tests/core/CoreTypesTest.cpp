...
{
    auto result = ToolResult::err(errors::FILE_NOT_FOUND,
                                  QStringLiteral("file.txt not found"));
...
{
    auto ev = RuntimeEvent::permissionRequest(
        QStringLiteral("req_001"),
        QStringLiteral("FileWriteTool"),
...
{
    auto ev = RuntimeEvent::taskState(TaskState::Completed,
                                      QStringLiteral("Done"));
...
{
    auto ev = RuntimeEvent::error(errors::PROVIDER_TIMEOUT,
